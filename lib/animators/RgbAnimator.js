...
class RgbAnimator {
  ...
  parsefromValue() {
    if (this.fromValue == null) {
      ...
      this.fromValue = rgbRegex.exec(this.options.from).slice(1).map(value => new Number(value).valueOf());
    }
  }
  ...
  parsetoValue() {
    if (this.toValue == null) {
      rgbRegex.lastIndex = 0;
      this.toValue = rgbRegex.exec(this.options.to).slice(1).map(value => new Number(value).valueOf());
    }
  }
