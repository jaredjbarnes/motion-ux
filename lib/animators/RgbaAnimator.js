...
class RgbaAnimator {
  ...
  parsefromValue() {
    if (this.fromValue == null) {
      ...
      this.fromValue = rgbaRegex.exec(this.options.from).slice(1).map(value => new Number(value).valueOf());
    }
  }
  ...
  parsetoValue() {
    if (this.toValue == null) {
      rgbaRegex.lastIndex = 0;
      this.toValue = rgbaRegex.exec(this.options.to).slice(1).map(value => new Number(value).valueOf());
    }
  }
