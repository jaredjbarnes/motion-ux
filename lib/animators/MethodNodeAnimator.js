...
function () {
  ...
  _createClass(MethodNodeAnimator, [{
    ...
    value: function createArgs() {
      this.fromArgs = this.options.fromNode.children.find(function (node) {
        return node.name === "arguments";
      }).children.filter(function (node) {
        return node.name === "values";
      });
      this.toArgs = this.options.toNode.children.find(function (node) {
        return node.name === "arguments";
      }).children.filter(function (node) {
        return node.name === "values";
      });
    ...
    value: function createAnimators() {
      ...
      this.animators = this.fromArgs.map(function (node, index) {
        return new _ValuesNodeAnimator.default(_objectSpread({}, _this.options, {
          fromNode: node,
          toNode: _this.toArgs[index]
        }));
      });
    }
  }, {
    ...
    value: function getMethodName() {
      return this.options.fromNode.children.find(function (node) {
