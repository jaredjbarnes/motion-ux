...
class FunctionAnimator {
    ...
    if (this.fromValue == null) {
      ...
      while (result = functionRegEx.exec(this.options.from)) {
        ...
        result[2].split(",").forEach(value => {
          ...
          const valueResult = _unitRegEx.default.exec(value.trim());
    ...
    if (this.change == null) {
      Object.keys(this.functionNames).forEach(key => {
        ...
        if (functionData.fromValues.length !== functionData.toValues.length) {
          throw new Error("Invalid Arguments: The from values length don't match the to values length.");
        }
  ...
  calculateProgress() {
    ...
    this.value = Object.keys(this.functionNames).map(key => {
      const functionData = this.functionNames[key];
      ...
      const values = functionData.fromValues.map((from, index) => {
        return "".concat(from + easingProgress * functionData.change[index]).concat(functionData.fromUnits[index]);
      }).join(", ");
