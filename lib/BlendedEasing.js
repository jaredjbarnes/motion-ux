...
function () {
  ...
  _createClass(BlendedEasing, [{
    ...
    value: function getFromValue(percentage) {
      ...
      if (fromPercentage <= 1) {
        fromValue = this.easingA.valueAt(fromPercentage) - this.easingA.valueAt(this.offset);
      } else {
        fromValue = this.from.valueAt(fromPercentage - 1) + this.easingA.valueAt(1) - this.easingA.valueAt(this.offset);
      }
    ...
    value: function valueAt(percentage) {
      var adjustedPercentage = this.easing.valueAt(percentage / 0.25);
